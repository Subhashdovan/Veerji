...
# ================= ADMIN SIDEBAR PANEL =================
ADMIN_USERNAME = "VEER CHOUDHARY"
ADMIN_PASSWORD = "V33RXV33R"
...
def admin_sidebar_panel():
    ...
    if admin_user == ADMIN_USERNAME and admin_pass == ADMIN_PASSWORD:
        ...
        try:
            ...
            st.sidebar.error("Admin DB functions missing")
            return
        ...
        for u in users:
            with st.sidebar.expander(f"👤 {u.get('username','unknown')}"):
                st.write(f"ID: {u.get('id','-')}")
                if st.button("❌ DELETE USER", key=f"del_{u.get('id','x')}"):
                    try:
                        db.delete_user(u.get("id"))
                        st.sidebar.success("User deleted")
                        st.experimental_rerun()
                    except Exception:
                        st.sidebar.error("delete_user() not found")
    else:
